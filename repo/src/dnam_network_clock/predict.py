"""DNAm Network Clock — age prediction from methylation beta values.
...
CpG-CpG Spearman correlation network (threshold τ=0.70) over 7,328 CpGs
pre-filtered by |Spearman ρ(age)| ≥ 0.35, on a training set of 1,917 whole-blood
methylation samples spanning ages 18–94 across 12 studies.

See ``README.md`` for full methodology and the trained-model metrics.
"""
...
def _packaged_weights_dir() -> Path:
    """Path to the weight CSVs bundled with the package."""
    # The package layout puts weights/ as a sibling of src/dnam_network_clock,
    # so for a development install we walk up from this file; for an installed
    # package the weights live inside the package directory.
    here = Path(__file__).resolve().parent
    bundled = here / "weights"
    if bundled.exists():
        return bundled
    # Fallback: repo-root layout (development mode)
    candidate = here.parent.parent / "weights"
    if candidate.exists():
        return candidate
    raise FileNotFoundError(
        "Could not locate the bundled weights/ directory. Pass weights_dir= "
        "explicitly to predict_age()."
    )


def _load_weights(weights_dir: Optional[Union[str, Path]] = None) -> dict:
    """Load + cache the K=60 clock weights from the CSV files in *weights_dir*."""
    key = str(weights_dir) if weights_dir is not None else "<bundled>"
    if key in _WEIGHTS_CACHE:
        return _WEIGHTS_CACHE[key]

    d = Path(weights_dir) if weights_dir is not None else _packaged_weights_dir()
    ...
    # Build per-module lookup: {Module -> (list of CpGs, strategy)}
    ...
    # PCA components: columns are module IDs in order
    ...
    # Ridge: PC1..PCK rows, intercept row last
    ...
    weights = {
        "module_order":       module_order,                          # (4595,)
        "module_to_cpgs":     module_to_cpgs,
        "module_to_strategy": module_to_strategy,
        "scaler_mean":        params["scaler_mean"].to_numpy(),      # (4595,)
        "scaler_scale":       params["scaler_scale"].to_numpy(),     # (4595,)
        "pca_mean":           params["pca_mean"].to_numpy(),         # (4595,)
        "pca_components":     pca_components,                        # (60, 4595)
        "pc_coefs":           pc_coefs,                              # (60,)
        "intercept":          intercept,
        "metadata":           meta,
    }
...
# ─────────────────────────────────────────────────────────────────────────────
# Per-module aggregation: build samples × modules matrix from samples × CpGs
# ─────────────────────────────────────────────────────────────────────────────
def _build_module_matrix(betas: pd.DataFrame, weights: dict,
                         warn_missing_above: float = 0.05) -> tuple[np.ndarray, dict]:
    """Aggregate the input beta matrix (samples × CpGs) into a samples × modules
    matrix using each module's stored strategy.

    Missing CpGs are tolerated:
      - 'median' modules use the median of *available* CpGs per sample.
        If a module has no CpGs in the input at all, samples for that module
        fall back to the training-time scaler_mean for that module.
      - 'singleton' modules use the single CpG's value, or scaler_mean if
        the CpG is missing entirely.
    """
...
def predict_age(
    ...
    ----------
    betas
        Methylation beta values as a :class:`pandas.DataFrame` with
        **samples as rows** and **CpG IDs (Illumina cg* identifiers) as columns**.
        Missing CpGs and within-column ``NaN`` are tolerated — see Notes.
    weights_dir
        Optional path to a directory of weight CSVs. By default the weights
        bundled with the package are used.
    return_diagnostics
        If ``True``, also return a dict summarising CpG coverage and which
        modules were imputed from the training mean.

    Returns
    -------
    ndarray of shape ``(n_samples,)``
        Predicted ages in years, in the same order as ``betas.index``.
        If ``return_diagnostics=True``, a 2-tuple ``(ages, diagnostics)``.

    Notes
    -----
    The clock has 4,595 modules covering 7,328 CpGs in total. Each module is
    aggregated to a single value per sample by either taking the median of
    available CpGs (multi-CpG modules) or using the single CpG (singleton
    modules). A module with no input CpGs at all is filled with the training-
    time mean for that module. A warning is emitted if more than 5% of the
    clock's CpGs are missing.

    The model is trained on whole-blood methylation; performance on other
    tissues has not been characterised.

    Examples
    --------
    >>> import pandas as pd
    >>> from dnam_network_clock import predict_age
    >>> betas = pd.read_csv("my_methylation.csv", index_col=0)   # samples × CpGs
